...
function run_on_mon {
    INV_C=~/overcloud-deploy/ceph-e/tripleo-ansible-inventory.yaml 
    ...
    ansible --private-key /home/stack/.ssh/id_rsa_tripleo -i $INV_C oc0-ceph-0 -b -m shell -a "$CMD"
}
...
if [ $KEYS -eq 1 ]; then
    ...
    NOV=/var/lib/config-data/puppet-generated/nova_libvirt/etc/nova
    ...
    ansible -i $INV Controller,Compute -b -m shell -a \
        "cat $DST/ceph.client.openstack.keyring"

    # Nova secret
    #ansible -i $INV Compute -b -m shell -a "ls $NOV/"
    #ansible -i $INV Compute -b -m shell -a "cat $NOV/secret.xml"
fi
